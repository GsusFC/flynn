...
import { generateStaticSVG, generateAnimatedSVG } from '../export/svgGenerator';
import { generateGIFFromVectors } from '../export/gifGenerator';
...
export const useSimpleVectorGrid = ({
  ...
  const exportAnimatedSVG = useCallback(async (config: Partial<ExportConfig> = {}): Promise<string> => {
    if (!isClient) throw new Error('Export only available on client side');
    ...
    try {
      ...
      const frameCount = config.fps ? Math.ceil((cycle.duration / 1000) * config.fps) : cycle.frameCount;
      const vectorFrames: SimpleVector[][] = [];
      
      // Capturar frames
      for (let i = 0; i < frameCount; i++) {
        const timestamp = (i / frameCount) * cycle.duration;
        const vectors = getVectorsAtTime(timestamp);
        vectorFrames.push(vectors);
